...
protected:
  ...
  void SetUp() {
    ...
    specManager = dynamic_cast<d4::SpecManagerAll *>(d4::SpecManager::makeSpecManager(config, *d4::ProblemManager::makeProblemManager(config, out), out));
    }
};
...
protected:
  ...
  void SetUp() {
    d4::Config config;
    config.input = "instancesTest/cnfs_and_alternatives/cnfConnectedComponent1.cnf";
    config.input_type = "cnf";
    config.alternative_input =
        "instancesTest/cnfs_and_alternatives/alternativeConnectedComponent1.cnf";
    config.occurrence_manager = "dynamic";
    std::ostringstream out;
    specManager = dynamic_cast<d4::SpecManagerAll *>(d4::SpecManager::makeSpecManager(config, *d4::ProblemManager::makeProblemManager(config, out), out));
    }
