...
template <class T> class BucketManagerAll : public BucketManager<T> {
protected:
  SpecManagerAll &m_specManager;
...
public:
  ...
  */
  BucketManagerAll(SpecManagerAll &occM, CacheManager<T> *cache,
  ...
  bool isKeptClause(int idx) {
    switch (m_modeStore) {
    case NT:
      return m_specManager.clauseAssignmentCount(idx);
    case NB:
