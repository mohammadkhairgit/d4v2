...
void ProblemManagerCnf::normalize() {
  if (m_selected.size() != m_nbVar) {
    ...
    int i = 1;
    // Mark projected variables to "move" them to the front of the variable
    // list.
    ...
    // With the projected variables indexed from 1 to n, where n is the number
    // of projected variables, we can now index the remaining variables.
    for (Var v = 1; v <= m_nbVar; v++) {
      if (!marked[v]) {
        remap[v] = i;
        i++;
    ...
    // Give the original literals "l" in a clause "c" their new index based on
    // their value in remap.
