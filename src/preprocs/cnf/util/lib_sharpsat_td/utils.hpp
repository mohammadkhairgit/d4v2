...
inline Lit Neg(Lit x) { return x ^ 1; }

inline Var VarOf(Lit x) { return x / 2; }

inline Lit PosLit(Var x) { return x * 2; }

inline Lit NegLit(Var x) { return x * 2 + 1; }
...
inline bool IsPos(Lit x) { return !(x & 1); }

inline bool IsNeg(Lit x) { return x & 1; }
...
template <typename T> void Shuffle(vector<T> &vec, std::mt19937 &gen) {
...
template <typename T> T Power2(int p) {
  assert(p >= 0);
  if (p == 0)
    return 1;
  if (p % 2 == 0) {
...
template <typename T> std::vector<T> PermInverse(const std::vector<T> &perm) {
...
template <typename T> void SortAndDedup(vector<T> &vec) {
...
template <typename T> void ShiftDel(vector<T> &vec, size_t i) {
  assert(i < vec.size());
  for (; i + 1 < vec.size(); i++) {
...
template <typename T> int Ind(const std::vector<T> &a, const T &x) {
  int ind = std::lower_bound(a.begin(), a.end(), x) - a.begin();
  assert(a[ind] == x);
  return ind;
}

template <typename T> bool BS(const std::vector<T> &a, const T x) {
...
inline Timer::Timer() {
  ...
  elapsedTime = std::chrono::duration<double>(
      std::chrono::duration_values<double>::zero());
}
