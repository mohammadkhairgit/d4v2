...
bool Subsumer::CSO1(const vector<vecP> &D, size_t b, size_t e, const vecP S,
  ...
  while (j < S.size() && getD(S, j) < getD(D[b], d))
    j++;
  if (j >= S.size())
    return false;
  ...
  if (getD(S, j) == getD(D[b], d)) {
    ...
    if (d == 0) {
      ...
      while (ee + 1 <= e && getD(D[ee + 1], d) == getD(S, j))
        ee++;
    } else {
      int mi = b;
      int ma = e - 1;
  ...
  if (b <= e)
    return CSO1(D, b, e, S, j, d, d0Index);
  return false;
}

vector<vector<Glucose::Lit>>
Subsumer::Subsume(const vector<vector<Glucose::Lit>> &clauses) {
  ...
  auto cmp = [&](vecP a, vecP b) {
    if (getD(a, 0) < getD(b, 0))
      return true;
    else if (getD(a, 0) > getD(b, 0))
      return false;
    size_t sz = min(a.size(), b.size());
    for (size_t i = 1; i < sz; i++) {
      if (getD(a, i) < getD(b, i))
        return true;
      else if (getD(a, i) > getD(b, i))
        return false;
    }
