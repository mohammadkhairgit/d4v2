...
SpecManagerAll::SpecManagerAll(ProblemManager &p) {
  ...
  for (unsigned i = 0; i < m_clauses.size(); i++) {
    ...
    /* Before filling the actual occurrence attributes m_dataOccurrenceMemory
    and m_occurrence, we first store the occurrence in a temporary vector of
    vectors , so that the management of the memory m_dataOccurrenceMemory is
    correctly done.
    */
  ...
  for (unsigned i = 0; i < occurrence.size(); i++) {
    ...
    for (auto const &idx : occList) {
      if (m_clauses[idx]->size() == 2)
        // Fill the next available memory for the binary clause occurrence and
        // move forward.
        ptr[m_occurrence[i].nbBin++] = idx;
      else
        // Fill the current end available memory for the non-binary clause
        // occurrence, and move backward.
        ptr[posNotBin--] = idx;
    }
  ...
  // Not really needed because it only optimize the first connected component
  // creation.
...
int SpecManagerAll::computeConnectedComponent(
  ...
  for (auto const &v : setOfVar) {
    ...
    for (unsigned i = 0; i < 2; i++) {
      ...
      // Pointers to the start and end of the occurrence list for the current
      // literal.
      IteratorIdxClause listIndex = m_occurrence[l.intern()].getClauses();
      // Go through each clause index in the occurrence list for the current
      // literal.
      for (int *ptr = listIndex.start; ptr != listIndex.end; ptr++) {
        ...
        if (!m_markView[idx]) {
          ...
          while (rootW != m_infoCluster[rootW].parent) {
            /**
             * TODO: I do not understand why we set the parent to the parent of
             * the parent, but only once?
             */
  ...
  // With the union-find structure built, create the components and set the free
  // variables with the help of the structure.
  for (auto const &v : setOfVar) {
    if (m_currentAssignment[v] != l_Undef)
      continue;
    ...
    // If the root does not belong to any component yet, create a new component
    // and push the root.
...
int SpecManagerAll::computeConnectedComponent(
    std::vector<ProjVars> &varConnected, std::vector<Var> &setOfVar,
    std::vector<Var> &freeVar) {
  /** TODO: Mohammad,: (I am waiting on this TODO, because I would like to know
    that everything is working correctly first.) This code is technically a copy
    of the other computeConnectedComponent with the only difference at the end.
    We can create multiple for loops were the other TODO is and instead have 2-3
    other "mini" functions instead of duplicating the code.
  */
  for (auto const v : setOfVar) {
    assert(v < m_infoCluster.size());
    m_infoCluster[v].parent = v;
  ...
  /*TODO: Mohammad,: To solve the code duplication, copy the function until here
    ("Union_find") and then create three for loops One that contain an if
    "m_infoCluster[v].parent == v && m_infoCluster[v].size == 1" continue and
    not the last part of the loop. This loop should be part of the first
    substitution function "Union_Find" The second loop can stay here it is the
    same loop but it only set the freevar The third loop is the one difference
    between the two functions and it is the last part of the for loop, which
    will be done in the third for loop.
    */
  // With the union-find structure built, create the components and set the free
  // variables with the help of the structure.
  for (auto const &v : setOfVar) {
...
void SpecManagerAll::preUpdate(std::vector<Lit> &lits) {
  ...
  // Go through each literal and set as assigned and removed satisfied clauses
  // occurrences from other literals and updating the watcher if the literal was
  // the current watcher of a clause.
  for (auto &l : lits) {
    assert(!litIsAssigned(l));
    m_currentAssignment[l.var()] = l.sign() ? l_False : l_True;

    // Update clauses positive literal occurrences and watcher if the literal
    // was the current watcher. Also remove the occurrences of satisfied clauses
    // from other literals.
    ...
    // Update clauses negative literal occurrences and watcher if the literal
    // was the current watcher. Also remove the occurrences of satisfied clauses
    // from other literals.
    for (unsigned i = 0; i < m_occurrence[(~l).intern()].nbNotBin; i++) {
      int idxCl = m_occurrence[(~l).intern()].notBin[i];
...
void SpecManagerAll::preUpdate(std::vector<Lit> &lits, std::vector<Lit> &pure) {
  m_reviewWatcher.resize(0);
...
int SpecManagerAll::getNbOccurrence(Lit l) {
